...
#define CS_PORT_PERI    SYSCTL_PERIPH_GPIOG
#define SSI_PORT        GPIO_PORTA_BASE
#define CS_PORT         GPIO_PORTG_BASE

#define SSI_CS          GPIO_PIN_0
...
{
 	unsigned long ulConfig = HWREG(SSI0_BASE + SSI_O_CR0);

	// verify that polarity and phase are set correctly for File IO communication
	if((ulConfig & (1 << 6)) || (ulConfig & (1 << 7)))
	{
		SSIDisable(SSI0_BASE);
	 	SSIConfig(SSI0_BASE, SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 12500000, 8);
		SSIEnable(SSI0_BASE);
	}
}
