/***************************************************************************************
	...
	MCB2400_AI.c - Functions to implement analog input (D2A).
...
uInt8 _gn2400AnalogChanEnable = 0;  

void MCB2400_AIInit(int32 nErrIn, uInt8 nChanEnable, int32* pnErrOut) {
	...
	if(nChanEnable != _gn2400AnalogChanEnable)
	{
	...
	if (nChanEnable & 0x1) { //AD0.0 enable  (P0.23)
		PINSEL1 &= ~0x8000;
		PINSEL1 |= 0x4000;
	}
	if (nChanEnable & 0x2) { //AD0.1 enable	 (P0.24)
		PINSEL1 &= ~0x20000;
		PINSEL1 |= 0x10000;
	}
	if (nChanEnable & 0x4) { //AD0.2 enable	 (P0.25)
		PINSEL1 &= ~0x80000;
		PINSEL1 |= 0x40000;
	}
	if (nChanEnable & 0x8) { //AD0.3 enable	 (P0.26)
		PINSEL1 &= ~0x200000;
		PINSEL1 |= 0x100000;
	}
	if (nChanEnable & 0x10) { //AD0.4 enable  (P1.30)
		PINSEL3 |= 0x30000000;
	}
	if (nChanEnable & 0x20) { //AD0.5 enable  (P1.31)
		PINSEL3 |= 0xC0000000;
	}
	if (nChanEnable & 0x40) { //AD0.6 enable   (P0.12)
		PINSEL0 |= 0x3000000;
	}
	if (nChanEnable & 0x80) { //AD0.7 enable   (P0.13)
		PINSEL0 |= 0xC000000;
	}
	...
	_gn2400AnalogChanEnable |= nChanEnable;
	}
...
void MCB2400_AnalogIn(uInt8 nChan, int32 nErrIn, uInt16 *pnValue, Boolean *pbOverrun, int32* pnErrOut) {	
	...
	if( (nChan > 7) || ((!_gn2400AnalogChanEnable)&(0x01 << nChan)) )  
	{
	 	if (pnErrOut) {
			*pnErrOut = 1;
		}
